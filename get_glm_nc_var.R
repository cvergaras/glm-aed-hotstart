                                  ...
                                  diagnostic_vars)
{
  ...
  if("PHY_cyano" %in% states){
    chla <- matrix(ncdf4::ncvar_get(glm_nc, "PHY_tchla"), ncol = final_time_step)[1:tallest_layer,
                                                                                  final_time_step]
  }
  ...
  return(list(output = output, output_no_depth = output_no_depth,
              lake_depth = heights_surf, heights = heights, snow_wice_bice = c(snow, ice_white, ice_blue),
              avg_surf_temp = avg_surf_temp, mixing_vars = mixing_vars,
              salt = salt, diagnostics_output = diagnostics_output,
              chla = chla, mixer_count = mixer_count))
}